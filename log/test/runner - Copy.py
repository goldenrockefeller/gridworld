...
def critic_update(args):
    ...
    for phenotype_id in range(n_phenotypes):
        ...
        if new_critic is not None:
            new_critic.update(observations, actions, rewards)

        phenotype["fitness"] = fitness

    return critic, new_critic, phenotypes
    ...
    def new_run(self):
        ...
        for epoch_id in range(n_epochs):
            ...
            if n_pools > 1:
                ...
                args_list = zip([domain  for phenotype_id in range(n_policies)], moving_policies_x_phenotype, targetting_policies_x_phenotype)
                trajectories_x_phenotype, records_x_phenotype = zip(*p.map(domain_execute, args_list))
                ...
                args_list = (
                    zip(
                        ...
                        targetting_critics + moving_critics,
                        new_targetting_critics + new_moving_critics,
                        targetting_phenotypes_x_robot + moving_phenotypes_x_robot,
                    )
                )
                critics_x_robot, new_critic_x_robot, phenotypes_x_robot = zip(* p.map(critic_update, args_list))

                targetting_critics = critics_x_robot[:len(critics_x_robot)//2]
                new_targetting_critics = new_critic_x_robot[:len(critics_x_robot)//2]
                targetting_phenotypes_x_robot = phenotypes_x_robot[:len(critics_x_robot)//2]

                moving_critics = critics_x_robot[len(critics_x_robot)//2:]
                new_moving_critics = new_critic_x_robot[len(critics_x_robot)//2:]
                moving_phenotypes_x_robot = phenotypes_x_robot[len(critics_x_robot)//2:]
