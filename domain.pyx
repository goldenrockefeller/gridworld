...
all_moving_actions = list(MovingAction)
...
class Quandrant(IntEnum):
    BOTTOM_RIGHT = 0
    TOP_RIGHT = 1
    BOTTOM_LEFT = 2
    TOP_LEFT = 3

all_quandrants = list(Quandrant)
#quandrant = all_quandrants[is_target_to_the_top * 1 + is_target_to_the_left * 2]

Observation = Tuple[TargetType, Quandrant, int] # last value is "log-distance" to target



cdef Py_ssize_t random_cache_size = 10000
cdef Py_ssize_t random_uniform_counter = 0
cdef double[::1] np_random_uniform_cache = rng.random(random_cache_size)
...
cpdef double random_uniform() except *:
    ...
    if random_uniform_counter >= random_cache_size:
        ...
        np_random_uniform_cache = rng.random(random_cache_size)
...
cpdef list closest_robots(Robot robot, list prev_closest_robots, list robots):
    """
    Gets the order list of robots closest to 'robot', given the list of
    all robots 'robots', using 'prev_closest_robots' for tie-breaking.
    """
    ...
    # The following code tries not to change the order of the new list
    # from the previously closest robots list if there is a tie.
...
cpdef tuple observation(Robot robot): # -> Observation
    ...
    quadrant = all_quandrants[is_target_to_the_top * 1 + is_target_to_the_left * 2]
...
cpdef list goal_capturers(Pos goal, list robots, Py_ssize_t n_req):
    """
    If the goal is capture, return the robots that captured it
    """
    cdef list capturers = []
    cdef Robot robot
    for robot in robots:
...
cdef class Robot:
    ...
    def __init__(self, random_pos):
        ...
        self.obs_fail_rate = 0.
        self.moving_action_fail_rate = 0.
    ...
    def execute(
        ...
        for step_id in range(n_steps):
            ...
            for robot, trajectory, moving_policy, targetting_policy in zip(robots, trajectories, moving_policies, targetting_policies):
                ...
                moving_action = moving_policy.action(robot.observation, possible_moving_actions)
                ...
                target_type = targetting_policy.action(robot.observation, all_target_types)
            ...
            for goal_id, goal in enumerate(goals):
                the_goal_capturers = goal_capturers(goal, robots, n_req)
                if the_goal_capturers is not None:
                    reward += 1.
                    goals[goal_id] = random_pos(n_rows, n_cols)
                    # for robot in the_goal_capturers:
                    #     robot.deteriorate()
